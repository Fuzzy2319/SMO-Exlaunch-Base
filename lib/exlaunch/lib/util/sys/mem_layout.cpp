...
static void FindModules() {
    ...
    do {
        ...
        case State::ExpectingData: {
            ...
            curModInfo.m_Mod =
                reinterpret_cast<const Mod0*>(curModInfo.m_Text.m_Start + header->m_Mod0Offset);
...
static void FindRegions() {
    ...
    util::mem_layout::s_Heap = util::Range(GetAddressFromInfo(InfoType_HeapRegionAddress),
    ...
    if (R_FAILED(TryGetAddressFromInfo(InfoType_AslrRegionAddress, &mem_layout::s_Aslr.m_Start)) ||
        ...
        R_FAILED(
            TryGetAddressFromInfo(InfoType_StackRegionAddress, &mem_layout::s_Stack.m_Start)) ||
...
static void LogLayout() {
    ...
    for (size_t i = 0; i < static_cast<size_t>(ModuleIndex::End); i++) {
        ...
        Logging.Log(EXL_LOG_PREFIX "[ %-*s ]: \t%016lx-%016lx", maxModuleName, nameBuffer,
                    mod.m_Total.m_Start, mod.m_Total.GetEnd());
    }

    Logging.Log("");
    Logging.Log(EXL_LOG_PREFIX "Alias: \t%016lx-%016lx", mem_layout::s_Alias.m_Start,
                mem_layout::s_Alias.GetEnd());
    Logging.Log(EXL_LOG_PREFIX "Heap: \t%016lx-%016lx", mem_layout::s_Heap.m_Start,
                mem_layout::s_Heap.GetEnd());
    Logging.Log(EXL_LOG_PREFIX "Aslr: \t%016lx-%016lx", mem_layout::s_Aslr.m_Start,
                mem_layout::s_Aslr.GetEnd());
    Logging.Log(EXL_LOG_PREFIX "Stack: \t%016lx-%016lx", mem_layout::s_Stack.m_Start,
                mem_layout::s_Stack.GetEnd());
}
