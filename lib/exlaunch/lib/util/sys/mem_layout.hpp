...
struct Range {
    ...
    constexpr bool InRange(uintptr_t value) const { return m_Start <= value && value < GetEnd(); }
...
public:
    const Elf_Dyn* GetDynamic() const {
        return reinterpret_cast<const Elf_Dyn*>(GetPointer(m_DynamicStartOffset));
    }
    ...
    Range GetEhFrameHdr() const {
        return {GetEhFrameHdrStart(), GetEhFrameHdrEnd() - GetEhFrameHdrStart()};
    }
};
...
static inline const ModuleInfo& GetRtldModuleInfo() {
    return GetModuleInfo(ModuleIndex::Rtld);
}

static inline const ModuleInfo& GetMainModuleInfo() {
    return GetModuleInfo(ModuleIndex::Main);
}
...
inline const ModuleInfo* TryGetModule(uintptr_t pointer) {
    for (size_t i = static_cast<int>(ModuleIndex::Start); i < static_cast<int>(ModuleIndex::End);
         i++) {
