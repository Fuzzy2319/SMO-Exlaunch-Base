...
struct LookupEntryBin {
    ...
    constexpr bool operator<(const HashType& other) const { return m_SymbolHash < other; }
};
...
/* Would've liked to have used std::string_view here, but implementation demands this type be
 * "non-structural", so this is how it has to be. */
...
struct LookupEntry {
    ...
    constexpr LookupEntryBin Convert() const {
        return LookupEntryBin(util::Murmur3::Compute(GetSymbol()), m_Offset, m_ModuleIndex);
    }
};
}  // namespace impl
}  // namespace exl::reloc
