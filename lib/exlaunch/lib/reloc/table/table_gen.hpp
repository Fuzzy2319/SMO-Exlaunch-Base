...
constexpr bool IsUnique(In lookups) {
    return IsUnique(lookups, [](const auto& v) { return v; });
}
...
struct TableType {
    ...
    static_assert(impl::IsUnique(std::array<std::string_view, s_Size>{_Table.GetSymbol()...}),
                  "Duplicate symbol!");
...
static constexpr auto ExtractVersions(std::index_sequence<Indicies...>) {
    return std::array<VersionType, sizeof...(Indicies)>{
        (std::tuple_element_t<Indicies, Map>::s_Version)...};
}

constexpr size_t FindTableIndex(VersionType ver) {
    static constexpr std::array<VersionType, s_TableCount> versions =
        ExtractVersions(std::make_index_sequence<s_TableCount>{});
...
ALWAYS_INLINE TableBin GetTableByIndexImpl(size_t index, std::index_sequence<Indicies...>) {
    /* Check if any of the entries in the table have the same symbol hash. This can occur if there
     * happens to be a collision. */
    static_assert((true && ... &&
                   IsUnique(std::get<Indicies>(s_Map).m_Table,
                            [](const LookupEntryBin& v) { return v.m_SymbolHash; })),
                  "Symbol name hash collision!");
    /* Create array of spans to access tables at runtime by index. */
    static constexpr auto tables = std::array<TableBin, s_TableCount>{(TableBin(
        std::get<Indicies>(s_Map).m_Table.data(), std::get<Indicies>(s_Map).m_Table.size()))...};
