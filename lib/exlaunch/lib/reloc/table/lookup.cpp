...
struct RelAccessorBase {
    ...
    auto GetTarget() const { return reinterpret_cast<uintptr_t*>(m_Mod.module_base + GetOffset()); }
...
void ApplyRel(const LookupEntryBin& entry, RelAccessor<T> accessor, std::string_view symbolName) {
    ...
    if (
        ...
        type != ARCH_JUMP_SLOT && !(ARCH_IS_REL_ABSOLUTE(type)) && type != ARCH_GLOB_DAT) {
        return;
    }

    if (!util::HasModule(entry.m_ModuleIndex)) {
        Logging.Log(EXL_LOG_PREFIX "Symbol %s has invalid module index %d", symbolName.data(),
                    static_cast<int>(entry.m_ModuleIndex));
        return;
    }
    ...
    if (Logging.IsEnabled()) {
        ...
        Logging.Log(EXL_LOG_PREFIX "Symbol %s being relocated to [%s]+%x", symbolName.data(),
                    nameBuffer, entry.m_Offset);
    }

    /* Target address will be (referred module start + offset) + rel addend. */
    *target = static_cast<uintptr_t>(referredModule.m_Total.m_Start + entry.m_Offset +
                                     accessor.GetAddend());
}

template <typename T>
    requires IsElfRel<T>
void ApplyRels(const Lookup& table, const rtld::ModuleObject& mod, const T* ptr, size_t count,
               size_t size) {
...
void ApplyRels(const Lookup& table, const rtld::ModuleObject& mod) {
    ...
    /* Seems dkp's linker omits this tag, so RTLD ends up misunderstanding. Fortunately, this seems
     * to work out (?). */
...
void Lookup::Apply() const {
    ...
    modObj.Initialize(reinterpret_cast<char*>(info.m_Total.m_Start),
