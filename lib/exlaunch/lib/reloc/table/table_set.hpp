...
struct TableSet {
    static_assert((true && ... &&
                   std::is_same_v<VersionType, typename VersionedTables::VersionType>),
                  "Version type within table array does not match version of set!");
    ...
    ALWAYS_INLINE TableBin GetTableByIndexImpl(size_t index,
                                               std::index_sequence<Indicies...>) const {
        /* Check if any of the entries in the table have the same symbol hash. This can occur if
         * there happens to be a collision. */
        static_assert((true && ... &&
                       impl::IsUnique(std::get<Indicies>(s_Map).m_Table,
        ...
        static constexpr auto tables =
            std::array<TableBin, s_Count>{(TableBin(std::get<Indicies>(s_Map).m_Table.data(),
...
public:
    inline bool DoesTableExist(VersionType type) const { return FindTableIndex(type) != SIZE_MAX; }
