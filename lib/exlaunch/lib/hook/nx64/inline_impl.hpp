...
/* I should be able to do this, but GCC seems unhappy? Did I really manage to break the compiler?
 * Has to be some sort of achievement. */
/* Also yes I recognize how cursed this is. */
// template<size_t Length, char Type>
// using VectorAdapter = TypeCharAdapter<Type>::Type VECTOR_SIZE(TypeCharAdapter<Type>::Size *
// Length);

/* GCC seems to not be happy with implementing this properly (see above), so I have to specialize
 * each case :( */
...
struct GpRegisterAccessorImpl {
    GpRegisters& Get() { return *reinterpret_cast<GpRegisters*>(this); }
};

struct FloatRegisterAccessorImpl {
    FloatRegisters& Get() { return *reinterpret_cast<FloatRegisters*>(this); }
};
...
struct InlineFloatCtx : public InlineCtx {
    ...
    /*
        ...
        Pretty cursed, but I think this might make things easier for people who do not understand
       NEON (ie, people who touch grass).
    */
    ...
    ALWAYS_INLINE typename impl::VectorAdapter<Type, Length>::Type& V() {
        ...
        if constexpr (Length == 1) {
            if constexpr (Type == 'Q')
                ...
                static_assert(false, "Invalid type for 1 length vector!");
            ...
            if constexpr (Type == 'D')
                ...
                static_assert(false, "Invalid type for 2 length vector!");
            ...
            if constexpr (Type == 'S')
                ...
                static_assert(false, "Invalid type for 4 length vector!");
            ...
            //      return V64[Index].B;
                ...
                static_assert(false, "Invalid type for 8 length vector!");
