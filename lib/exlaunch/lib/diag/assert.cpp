...
inline NORETURN void AbortWithCtx(const AbortCtx& ctx) {
...
#endif
    {
        ...
        /* Have lower 32-bits of the address be the lower 32-bits of the value. This ensures users
         * can see (most of) the value even only given the bad address. */
...
AbortReason ToAbortReason(AssertionType type) {
    switch (type) {
    case AssertionType_Audit:
        return AbortReason_Audit;
    case AssertionType_Assert:
        return AbortReason_Assert;
    default:
...
void InvokeAssertionFailureHandler(const AssertionInfo& info) {
    ...
    const AbortInfo abort_info = {
        ToAbortReason(info.type), info.message, info.expr, info.func, info.file, info.line,
    };
...
void AbortImpl(const char* expr, const char* func, const char* file, int line) {
    ...
    const AbortInfo abort_info = {
        AbortReason_Abort, std::addressof(message), expr, func, file, line,
    };
...
NOINLINE void OnAssertionFailure(AssertionType type, const char* expr, const char* func,
    ...
    const AssertionInfo info = {
        type, std::addressof(message), expr, func, file, line,
    };
